  ...
  return (
    <div className={theme === "dark" ? "min-h-screen bg-slate-950 text-slate-100" : "min-h-screen bg-slate-50 text-slate-900"}>
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col gap-6 px-4 py-8">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-sm ${muted}`}>
                  Upload a meal photo and chat with AI. History and totals are below.
                </p>
              </div>
              <button
                onClick={toggleTheme}
        ...
        <section className={`flex flex-col gap-3 rounded-3xl border ${surface} p-4 shadow-sm`}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-semibold">History</h2>
              <p className={`text-sm ${muted}`}>Meals logged for the selected day</p>
            </div>
            <input
              type="date"
              value={selectedDate}
          ...
          <div className="rounded-2xl bg-blue-600 px-4 py-3 text-white shadow dark:bg-blue-500">
          ...
          <div className="flex max-h-[440px] flex-col gap-3 overflow-y-auto pr-1">
            {todaysEntries.length === 0 ? (
              <p className={`text-sm ${muted}`}>No meals logged yet. Add one above.</p>
            ) : (
              todaysEntries.map((entry) => (
                ...
                >
                  ...
                  />
                    ...
                    <div className="flex h-16 w-16 items-center justify-center rounded-xl bg-slate-200 text-sm text-slate-600 dark:bg-slate-700 dark:text-slate-100">
                      {entry.title[0]}
                    </div>
                  )}
